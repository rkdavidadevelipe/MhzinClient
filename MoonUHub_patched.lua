-- ======= MoonUHub Compat Shim (cole no topo do arquivo) =======
-- Compatibilidade: define executor, carrega UI com fallback e protege funções exploit-only.

-- 1) Executor detection (string)
local executor = (function()
    -- funções detectáveis por exploits comuns
    if type(identifyexecutor) == "function" then
        local ok, name = pcall(identifyexecutor)
        if ok and name then return tostring(name) end
    end
    if syn and type(syn.protect_gui) == "function" then return "Synapse" end
    if KRNL_LOADED then return "Krnl" end
    if secure_load then return "Secure" end
    -- fallback genérico
    return "UnknownExecutor"
end)()

-- 2) Safe get hui / coregui (alguns exploits usam gethui())
local function safe_get_ui_parent()
    if type(gethui) == "function" then
        local ok, res = pcall(gethui)
        if ok and res then return res end
    end
    -- syn.protect_gui can be used by Synapse to protect guis, wrap it if exists
    if syn and type(syn.protect_gui) == "function" then
        local screenGui = Instance.new("ScreenGui")
        screenGui.Name = "MoonUHubProtected"
...
local UIParent = safe_get_ui_parent()

-- 3) Safe sethiddenproperty wrapper
local function safe_sethiddenproperty(obj, prop, val)
    if type(sethiddenproperty) == "function" then
        local ok = pcall(function() sethiddenproperty(obj, prop, val) end)
        return ok
    end
    -- some exploits provide set_hidden_property alternative names; try generic pcall to avoid crash
    local ok, _ = pcall(function() obj[prop] = val end)
    return ok
end

-- 4) Try to load main UI library with fallback - não quebra se falhar
local Library = nil
do
    local tried = false
    -- tentativa principal (original)
    local mainURL = "https://raw.githubusercontent.com/tbao143/Library-ui/refs/heads/main/Redzhubui"
    local ok, res = pcall(function()
        local s = game:HttpGet(mainURL)
        return (type(s) == "string" and s ~= "") and loadstring(s)() or nil
    end)
    if ok and res then
        Library = res
        tried = true
    ...
    if not Library then
        -- fallback 1: Redzhubui/alternativa pública conhecida (substitua se preferir)
        local fallbackURLs = {
            "https://raw.githubusercontent.com/EdgeIY/infiniteyield/master/CustomLibrary", -- fallback genérico (pode não ser UI)
            "https://raw.githubusercontent.com/MeowMad/roblox-libraries/main/SomeUiLib" -- placeholder, não-fatal
        }
    ...
    if not Library then
        warn("[MoonUHub] Aviso: Biblioteca de UI não carregada. O script continuará, mas sem interface (ou com interface parcial).")
    end
end

-- 5) Pequeno utilitário para usar UIParent quando script cria guis
-- substitui chamadas gethui() usadas no script por UIParent, quando necessário.
-- Exemplo de uso (no resto do script): Gui.Parent = UIParent
